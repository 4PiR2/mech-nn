...
if ODEConfig.linear_solver == SolverType.LEAST_SQUARES:
    ...
    class ODEINDLayer(nn.Module):
        ...
        def __init__(self, bs, order, n_ind_dim, n_iv, n_step, n_iv_steps, solver_dbl=True, gamma=0.5, alpha=0.1,
                     central_diff=True, double_ret=False, device=None):
            super().__init__()
            self.n_step = n_step
            self.order = order
        ...
        def forward(self, coeffs, rhs, iv_rhs, steps):
            ...
            if iv_rhs is not None:
                ...
                iv_rhs = torch.empty(self.bs, self.n_ind_dim, 0, 1, self.n_iv, device=rhs.device)
            ...
            u0, u1, u2 = u[..., 0, :3].unbind(dim=-1)

            eps = torch.zeros(self.bs * self.n_ind_dim, dtype=rhs.dtype, device=rhs.device)
            steps = steps.reshape(self.bs * self.n_ind_dim, self.n_step - 1, 1)
            return u0, u1, u2, eps, steps


    class ODESYSLayer(nn.Module):
        def __init__(self, bs, order, n_ind_dim, n_dim, n_equations, n_iv, n_iv_steps, n_step, periodic_boundary=False,
                     solver_dbl=True, gamma=0.5, alpha=0.1, double_ret=True, device=None):
            super().__init__()
            self.n_step = n_step
            self.order = order
            self.n_dim = n_dim
        ...
        def forward(self, coeffs, rhs, iv_rhs, steps):
            coeffs = coeffs.reshape(self.bs, self.n_ind_dim, self.n_equations, self.n_step, self.n_dim,
                                    self.order + 1).transpose(-4, -3)
            ...
            if iv_rhs is not None:
                ...
                iv_rhs = torch.empty(self.bs, self.n_ind_dim, 0, self.n_dim, self.n_iv, device=rhs.device)
