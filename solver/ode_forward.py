...
def ode_forward(
    ...
    *batch_steps, n_steps_steps = steps.shape
    ...
    u = solve_linear.apply(block_diag_0, block_diag_1, block_diag_2, beta)
    u = u.reshape(*batch, n_steps, n_dims, n_orders)
    return u


class solve_linear(torch.autograd.Function):
    @staticmethod
    def forward(
            ctx,
        ...
        solve_linear.cholesky(block_diag_0_list, block_diag_1_list, block_diag_2_list)
        u = solve_linear.substitution(block_diag_0_list, block_diag_1_list, block_diag_2_list, beta)

        ctx.block_diag_0_list = block_diag_0_list
        ctx.block_diag_1_list = block_diag_1_list
        ctx.block_diag_2_list = block_diag_2_list
        ctx.u = u
        return u

    @staticmethod
    def backward(ctx, du: torch.Tensor):
    ...
    @staticmethod
    def cholesky(
        block_diag_0_list: List[torch.Tensor],
        block_diag_1_list: List[torch.Tensor],
        block_diag_2_list: List[torch.Tensor],
    ):
        # tensor shape: (..., n_dims * n_orders, n_dims * n_orders)
        n_steps: int = len(block_diag_0_list)
    ...
    def substitution(
        ...
        # tensor shape: (..., n_dims * n_orders, n_dims * n_orders)
        # beta shape: (..., n_steps, n_dims * n_orders, 1)
        n_steps: int = len(block_diag_0_list)
        # A X = B => L (Lt X) = B
        ...
        u: torch.Tensor = torch.stack(x_list, dim=-3)  # (..., n_steps, n_dims * n_orders, 1)
        return u
